...
1. **Build on the Qwak Platform**:
    ...
    ```bash
    qwak models build --model-id <your-model-id> ./codegen_conda --gpu-compatible --no-validate-serving-artifact
    ```
...
2. **Deploy the Model on the Qwak Platform with a Real-Time Endpoint**:
    ...
    ```bash
    qwak models deploy realtime --model-id <your-model-id> --build-id <your-build-id> --instance 'gpu.t4.xl' --timeout 50000
    ```
